...
object Token2BodyFeatureSequenceSvg {
  ...
  private def computeLayoutFeatures(lineInfos: Array[LineInfo], data: NewHtmlTokenizationSvg) {
    ...
    while (i < data.getLineSpans.size) {
      {
        val currentSpan: Span = data.getLineSpans(i) //.get(i)
        try {
      ...
      while (i < lineInfos.length) {
        {
          ...
          val lineSpan: Span = data.getLineSpans(lineSpanCount) //  .get(lineSpanCount)
  ...
  private def updateLineSpanCounter(data: NewHtmlTokenizationSvg, currentCounter: Int): Int = {
  ...
  private def computeLexiconFeatures(data: NewHtmlTokenizationSvg, dictionary: EnglishDictionary) {
...
class Token2BodyFeatureSequenceSvg extends Pipe {
  def pipe(carrier: Instance): Instance = {
    Token2BodyFeatureSequence.wordsInDictionaryPerLine = MutableList[LayoutUtils.Entry]()
    val data: NewHtmlTokenizationSvg = carrier.getData.asInstanceOf[NewHtmlTokenizationSvg]
    val nhtml2LineInfo: NewHtmlTokenization2LineInfoSvg = new NewHtmlTokenization2LineInfoSvg
  ...
  private def computeFeatures(lineInfos: Array[LineInfo], data: NewHtmlTokenizationSvg, dictionary: EnglishDictionary) {
    Token2BodyFeatureSequenceSvg.computeLexiconFeatures(data, dictionary)
    Token2BodyFeatureSequenceSvg.computeLayoutFeatures(lineInfos, data)
  }
}
