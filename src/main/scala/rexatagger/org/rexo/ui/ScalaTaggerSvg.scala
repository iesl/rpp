...
// */
//
//object ScalaTaggerSvg {
//
//  val logger = Logger(LoggerFactory.getLogger("ScalaTagger"))
//
//
//  val bodyAnnotations = List("paragraph", "section", "table", "figure");
//
...
//      val infile:String = line //files(0).trim
////      val outfile: File = new File(files(1).trim)
//
//      if ( infile != "") {
//        val document: List[Document] = readInputDocument(infile)
...
//
//
//    def writeOutput(outputPath: String, rdoc: RxDocumentSvg)
//    {
...
//
//      val annoReferences:List[Annotator] =
//          annotateV2("reference",'r', referencesTokenization,
//            rdoc.getTokenization._annotators)
//      val annoRefAndHeaders:List[Annotator] =
//        annotateV2("header",'h', headerTokenization,
//          annoReferences)
//      val annoRefHeadBody:List[Annotator] =
...
//
//      val annotatedReferences = MetaDataSvgAnnotator.annotateReferences(referencesTokenization, referenceLabels, annotatedBody)
//
//      annotatedReferences.zipWithIndex.map(x=>x._1.write(outputPath + (x._2 + 1) + ".svg"))
//
////      annoRefHeadBody.zipWithIndex.map(x=>x._1.write(outputPath + (x._2 + 1) + ".svg"))
//
//
//    }
//
...
//                 annotators:List[Annotator]):List[Annotator] =
//    {
...
//          if(resSlidingMap.contains(blockIndex + "_" + (x._2 + 1))){
//            resSlidingMap(blockIndex + "_" + (x._2 + 1))
//          }
//          else
//          {
//            None
